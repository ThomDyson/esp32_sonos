...
void setup_startup_screen_lvgl_elements() {
  ...
  lv_obj_set_style_bg_color(startupScreen, lv_color_hex(0xdddddd), LV_PART_MAIN);
  //lv_obj_add_event_cb(startupScreen, startup_screen_touch_cb, LV_EVENT_CLICKED, NULL);
  ...
  closeStartupScreenButtonLabel = create_button_label(closeStartupScreenButton, LV_SYMBOL_CLOSE);
  ...
  for (int i = 0; i < totalSonosCount; i++) {
    lv_obj_t *btn = lv_list_add_btn(sonos_list, NULL, sonosDevices[i].title);
    ...
    lv_obj_add_style(btn, &RoomSelectorLabelStyle, 0);
    lv_obj_add_style(label, &RoomSelectorLabelStyle, 0);
  }

 
  /*
    // Store the IP address in the button’s user data
  //  lv_obj_set_user_data(btn, (void *)sonosDevices[i].ip);

    // Add a label for IP below the button
   // lv_obj_t *label = lv_label_create(btn);
   // lv_label_set_text_fmt(label, "%IP: %s", sonosDevices[i].ip);
...
void setup_controller_screen_lvgl_elements() {
  ...
  lv_obj_set_style_bg_color(controllerScreen, lv_color_hex(0xdddddd), LV_PART_MAIN);
  ...
  lv_obj_set_size(trackTimerSlider, screenWidth * .55, 5);  // Set width and thin height
  ...
  lv_obj_set_style_bg_opa(trackTimerSlider, LV_OPA_TRANSP, LV_PART_KNOB);  // Hide the knob
  ...
  //lv_obj_add_flag(roomSelectorButton, LV_OBJ_FLAG_CLICKABLE);
  ...
  if (isRoundDisplay) {
    ...
    lv_obj_add_event_cb(startupScreenSelectorButton, startup_screen_selector_button_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    startupScreenSelectorButton = create_button(controllerScreen, startup_screen_selector_button_event_cb, lv_color_black(), 100, 100);
  ...
  lv_obj_add_event_cb(volumeAdjustment, volumeAdjustment_value_changed_event_cb, LV_EVENT_VALUE_CHANGED, NULL);
  lv_style_init(&volumeAdjustmentStyle);
  //lv_style_set_arc_color(&volumeAdjustmentStyle, lv_palette_main(LV_PALETTE_RED));
  lv_obj_add_style(volumeAdjustment, &volumeAdjustmentStyle, LV_PART_INDICATOR);

  sonosPlayStateButton = create_button(controllerScreen, sonosPlayStateButton_event_cb, lv_color_hex(0xFF5733), 0, 80);
  sonosPlayNextButton = create_button(controllerScreen, sonosPlayNextButton_event_cb, lv_color_hex(0x33A1FF), 110, 80);
  sonosPlayPreviousButton = create_button(controllerScreen, sonosPlayPreviousButton_event_cb, lv_color_hex(0x33A1FF), -110, 80);
  ...
  startupScreenSelectorLabel = create_button_label(startupScreenSelectorButton, LV_SYMBOL_LIST);
  // lv_obj_add_style(startupScreenSelectorLabel, &ButtonLabelStyleSmall, 0);

  lv_obj_add_event_cb(controllerScreen, controller_screen_touch_cb, LV_EVENT_CLICKED, NULL);
...
void updateDisplay() {
  ...
  if (isRoundDisplay) {
    lv_style_set_arc_color(&volumeAdjustmentStyle, lv_palette_main(playerIsMute ? LV_PALETTE_RED : LV_PALETTE_BLUE));
  } else {
    lv_style_set_bg_color(&volumeAdjustmentStyle, lv_palette_main(playerIsMute ? LV_PALETTE_RED : LV_PALETTE_CYAN));
  }
  ...
  int currentPercent = round(100 * trackPositionSeconds / (1 + trackDurationSeconds));  //div by zero bad
